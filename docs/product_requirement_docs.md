...
### 8. Password Management Features

The application includes comprehensive password management capabilities for handling password-protected PDF files:

#### 8.1. Password Field Highlighting

- **Visual Indicators:** Password fields are automatically highlighted with a yellow background and border when a password-protected PDF is selected.
- **Tooltips:** Informative tooltips appear when hovering over highlighted password fields, indicating that a password is required.
- **Real-time Updates:** Highlighting is applied immediately when files are added to the table.

#### 8.2. Saved Password Management

- **Password Storage:** Users can save frequently used passwords with descriptive names for quick access.
- **Password Selection:** A dropdown menu allows users to quickly select and apply saved passwords to files.
- **Password Management Dialog:** A dedicated dialog provides full CRUD operations for managing saved passwords:
  - Add new passwords with custom names
  - Edit existing password entries
  - Delete unwanted password entries
  - View creation and last-used timestamps
- **Persistent Storage:** Saved passwords are stored locally and persist between application sessions.
- **Security:** All password data is stored locally on the user's machine for privacy and security.

#### 8.3. User Interface Integration

- **Toolbar Integration:** Password management controls are integrated into the main toolbar for easy access.
- **Table Integration:** Password fields in the file table support both manual entry and selection from saved passwords.
- **Visual Feedback:** Toast notifications provide feedback for password management operations.
